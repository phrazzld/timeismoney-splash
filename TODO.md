...
- [ ] **T004 · Chore · P1: enhance eslint configuration (`eslint.config.mjs`)**
  ...
  - **Action:**
    ...
    4. Integrate `eslint-config-prettier` using `FlatCompat` as the _last_ configuration item.
  ...
  - **Verification:**
    ...
    3. Temporarily add a formatting issue that ESLint might catch but Prettier allows/fixes differently; verify ESLint does _not_ report it due to `eslint-config-prettier`.
...
- [x] **T007 · Chore · P1: configure husky pre-commit hook**
