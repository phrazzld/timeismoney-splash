...
describe('Scroll Utilities', () => {
  ...
  describe('scrollToElement', () => {
    ...
    test('throws error for null element reference', async () => {
      await expect(scrollToElement(null as unknown)).rejects.toThrow(
        'Invalid element: element cannot be null or undefined',
      );
    });

    test('handles scrollIntoView not supported gracefully', async () => {
      const elementWithoutScroll = { scrollIntoView: undefined } as unknown;
  ...
  describe('getScrollPosition', () => {
    test('returns current scroll position', () => {
      ...
      (window as unknown).scrollY = 150;
      (window as unknown).scrollX = 25;
    ...
    test('handles edge case when scroll height equals viewport height', () => {
      (window as unknown).scrollY = 0;
      (window as unknown).scrollX = 0;
    ...
    test('handles maximum scroll position', () => {
      (window as unknown).scrollY = 400; // 1000 - 600 = 400 max scroll
  ...
  describe('Error Handling', () => {
    ...
    test('handles elements without getBoundingClientRect', async () => {
      const elementWithoutRect = { getBoundingClientRect: undefined } as unknown;
